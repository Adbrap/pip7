...
def envoie_ticker(ticker,time1,time_name1):
    ...
    with open('mots.txt', 'r+') as file:
        ...
        if existing_content and not existing_content.endswith('\n'):
            file.write('\n')  # Ajouter un retour à la ligne s'il n'y en a pas déjà

        file.write('\n'.join(words))  # Écrire les mots sur des lignes séparées

    # Envoi du fichier modifié vers le serveur FTP
...
def Finder_IETE(time1, time_name1, start1):
    ...
    if passed1 == True:
        ...
        try:
            ...
            B = float(df['c'].iloc[local_min[-2]])
            C = float(df['c'].iloc[local_max[-2]])
            D = float(df['c'].iloc[local_min[-1]])
        ...
        if passed2 == True:
            ...
            if passed3 == True:
                ...
                ordre = False
                if local_max[-3] < local_min[-2] < local_max[-2] < local_min[-1] < local_max[-1]:
                    ordre = True
                # ----- condition pour que l'ordre des point de la figure soit respecter -----#
...
#
                        ...
                        try:
                            ...
                            plt.text(local_min[-2], B, "B", ha='left', style='normal', size=10.5, color='red', wrap=True)
                            plt.text(local_max[-2], C, "C", ha='left', style='normal', size=10.5, color='red', wrap=True)
                            plt.text(local_min[-1], D, f"D {round(D, 5)}", ha='left', style='normal', size=10.5, color='red', wrap=True)
                            ...
                            plt.scatter(local_min[-2], B, color='blue')
